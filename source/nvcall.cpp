...
struct Account {
	...
	void showName() { std::cout << "Account name is: " << name << std::endl; }
...
struct UserAccount : Account {
	...
	void adminStuff() { std::cout << "Admin Work not permitted for a user account!" << std::endl; }
};
...
struct AdminAccount : Account {
	...
	void adminStuff() { std::cout << "Would do admin work in context of: " << this->name << std::endl; }
};
...
int main(int argc, const char *argv[]) {
	...
	// imagine this change happens via memory corruption, type confusion, deserialization vulnerability,
	// or a similar flaw
